...
fn by_month_rows_to_chart(rows: &[database::ByMonthRow]) -> Result<charming::Chart> {
    ...
    // We assume the data is already sorted by month, descending, so we invert it!
    ...
    while i < rows.len() {
        ...
        // Initialize all series for this month with 0.0
        for account_name in &accounts {
            for series in [
                &mut account_to_balance_series,
        ...
        while i < rows.len() {
            ...
            // Replace the 0.0 with real information, if the account actually has any for this
            // month.
...
fn by_tag_rows_to_chart(rows: &[database::ByTagRow]) -> Result<charming::Chart> {
    let debits = rows
        .iter()
        .filter_map(|r| {
            let normalized_tag = if r.tag == "" { "(untagged)" } else { &r.tag };
            if r.tag_debit_pct >= 1.0 {
                Some((r.tag_debit_pct, normalized_tag))
            } else {
                None
            }
        })
    ...
    let credits = rows
        ...
        .filter_map(|r| {
            let normalized_tag = if r.tag == "" { "(untagged)" } else { &r.tag };
            if r.tag_credit_pct >= 1.0 {
                Some((r.tag_credit_pct, normalized_tag))
            } else {
                None
            }
        })
...
pub fn render_by_month_chart<W>(writer: &mut W, rows: &[database::ByMonthRow]) -> Result<()>
where
    W: Write,
{
    chart_to_sixel(writer, by_month_rows_to_chart(rows)?)
}
