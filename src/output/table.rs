    ...
    W: Write,
{
    fn new(writer: W, termwidth: Option<usize>) -> TableOutput<W> {
        TableOutput {
            writer: writer,
            termwidth: termwidth.unwrap_or(textwrap::termwidth()),
        }
    }
    ...
    W: Write,
{
    ...
    where
        W: Write,
    {
        writeln!(self.writer, "{:=^width$}", "", width = table_width)?;
        self.render_row(&columns, column_widths)?;
        writeln!(self.writer, "{:=^width$}", "", width = table_width)?;
        Ok(())
    }

    fn render_row(&mut self, row_contents: &Vec<Vec<String>>, column_widths: &[usize]) -> Result<()>
    where
        W: Write,
    {
        ...
        for line_number in 0..row_height_in_lines {
            for (column_idx, lines_in_cell) in row_contents.iter().enumerate() {
                write!(self.writer, "{}", if line_number == 0 { "|" } else { ":" })?;
                if line_number < lines_in_cell.len() {
                    ...
                    write!(self.writer, "{:^pad_left$}{line}{:^pad_right$}", "", "")?;
                } else {
                    write!(
                        self.writer,
                        "{:^width$.width$}",
            ...
            writeln!(self.writer, "{}", if line_number == 0 { "|" } else { ":" })?;
        }

        Ok(())
    }

    pub fn render_table<C, R, S>(&mut self, columns: &[C], rows: &[R]) -> Result<OutputForTesting>
    where
        C: AsRef<str>,
        R: AsRef<[S]>,
        S: AsRef<str>,
    {
        let output_for_testing = (
            columns.iter().map(|c| c.as_ref().to_string()).collect(),
            rows.iter()
                .map(|r| r.as_ref().iter().map(|i| i.as_ref().to_string()).collect())
                .collect(),
        );

        let n_columns = columns.len();
        // Maximum overall table width, with borders.
        let max_table_width = self.termwidth;
        ...
        assert!(table_width <= self.termwidth);

        setup_pager();
        self.render_header(&columns, table_width, &column_widths)?;

        let mut row = 0;
        for row_contents in &rows {
            self.render_row(row_contents, &column_widths)?;
            row += 1;
            ...
            if row % 20 == 0 {
                ...
                while col < table_width - formatted_row_number.len() {
                    write!(self.writer, "{}", if col % 2 == 0 { "." } else { " " })?;
                    col += 1;
                }
                writeln!(self.writer, "{formatted_row_number}")?;
            }
        }
        ...
        Ok(OutputForTesting::Table(output_for_testing))
    }
}
...
mod render_table_tests {
    ...
    fn simple_cell() {
        ...
        let columns = ["Column"];
        let rows = [["Cell"]];
        let mut to = TableOutput::new(cursor.get_mut(), Some(TERMWIDTH));
        to.render_table(&columns, &rows).unwrap();
